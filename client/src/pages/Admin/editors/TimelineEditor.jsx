...
const TimelineEditor = () => {
  ...
  useEffect(() => {
    if (portfolioData?.timeline) setList(portfolioData.timeline);
  }, [portfolioData]);
  ...
  const addItem = () => setList([...list, { year: '', event: '' }]);
  ...
  const handleSave = async (e) => {
    ...
    setStatus('Saving...');
    try {
      await api.updateSection('timeline', list);
      await refreshData();
  ...
  return (
    <div style={{ maxWidth: '900px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <h2 style={{ fontFamily: 'var(--font-heading)', color: 'var(--text-primary)', fontSize: '2rem' }}>Edit Timeline</h2>
        <button onClick={addItem} style={{ padding: '0.5rem 1rem', border: '1px solid var(--accent-gold)', color: 'var(--accent-gold)', fontFamily: 'var(--font-mono)' }}>+ ADD EVENT</button>
      </div>
      ...
      <form onSubmit={handleSave}>
        {list.map((item, index) => (
          <div key={index} style={{ padding: '2rem', backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border-light)', marginBottom: '2rem', position: 'relative' }}>
            ...
            <textarea value={item.event} onChange={(e) => handleItemChange(index, 'event', e.target.value)} style={{ ...inputStyle, minHeight: '80px', resize: 'vertical' }} required />
          </div>
        ))}
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <button type="submit" style={{ padding: '1rem 2rem', backgroundColor: 'var(--accent-gold)', color: '#0f0f0d', fontFamily: 'var(--font-mono)', fontWeight: 600 }}>SAVE TIMELINE</button>
